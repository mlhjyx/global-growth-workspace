        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-foreground-500 text-xs mt-0.5">
              基于 ICP 发现、评分与确认合格线索（Qualified Lead）
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-1.5 md:gap-4 text-[11px] md:text-xs">
          <div className="flex items-center gap-1.5 text-foreground-500">
            <span className="w-3.5 h-3.5 flex items-center justify-center">
              <i className="ri-user-search-line text-xs"></i>
            </span>
            <span>
              活跃 ICP:{' '}
              <span className="text-foreground-300">{mockICPs.filter((i) => i.active).length}</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-foreground-500">
            <span className="w-3.5 h-3.5 flex items-center justify-center">
            ...
            <span>
              线索: <span className="text-foreground-300">{mockAccounts.length}</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-foreground-500">
            <span className="w-3.5 h-3.5 flex items-center justify-center">
            ...
            <span>
              ...
              <span className="text-foreground-300">
                {mockSignalEvents.filter((s) => !s.read).length}
              </span>
            </span>
          </div>
        </div>
