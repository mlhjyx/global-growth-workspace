...
export default function ContentPage() {
  ...
  const selectedContent =
    mockContentItems.find((c) => c.id === selectedContentId) || mockContentItems[0];
    ...
    <div className="flex flex-col md:h-[calc(100vh-3.5rem)]">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {/* 发布归属内容域（母本 6.2 一级导航六项），发布中心作为内容域子入口 */}
          <Link
            to="/publish"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-primary-300 border border-primary-500/30 hover:bg-primary-500/10 transition-colors whitespace-nowrap"
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5 md:gap-4 text-[11px] md:text-xs">
          <div className="flex items-center gap-1.5 text-foreground-500">
            ...
            <span>
              内容总数: <span className="text-foreground-300">{mockContentItems.length}</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-foreground-500">
            <span className="w-3.5 h-3.5 flex items-center justify-center">
              <i className="ri-translate text-xs"></i>
            </span>
            <span>
              本地化条目: <span className="text-foreground-300">{mockLocalizations.length}</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-foreground-500">
            <span className="w-3.5 h-3.5 flex items-center justify-center">
            ...
            <span>
              ...
              <span className="text-warning">
                {mockComplianceIssues.filter((i) => i.severity === 'critical').length} 严重
              </span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-foreground-500">
