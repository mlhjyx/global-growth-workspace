...
export default function ContentComparison({ localizations, complianceIssues, selectedContentTitle }: Props) {
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              ...
              <div className="flex-1 flex flex-col border-r border-b md:border-b-0 border-primary-500/10 min-w-0 min-h-[160px]">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  <p className="text-foreground-200 text-sm leading-relaxed whitespace-pre-wrap">{selectedLoc.sourceText}</p>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col border-r border-b md:border-b-0 border-primary-500/10 min-w-0 min-h-[160px]">
                <div className="px-4 py-2.5 border-b border-primary-500/10 shrink-0 flex items-center justify-between">
                  <span className="text-primary-300 text-[11px] font-medium">
                    适配草案 <span className="text-foreground-600">({selectedLoc.targetLanguage.toUpperCase()} · {selectedLoc.targetMarket})</span>
                  </span>
                  <span className={`text-[10px] px-1.5 py-0.5 border rounded ${modeColors[selectedLoc.mode]}`}>
                    {modeLabels[selectedLoc.mode]}
                  </span>
                </div>
                <div className="flex-1 overflow-y-auto p-4">
                  <p className="text-foreground-100 text-sm leading-relaxed whitespace-pre-wrap">{selectedLoc.adaptedText}</p>
                </div>
              </div>
              ...
              <div className="w-full md:w-[280px] flex flex-col shrink-0 min-w-0 min-h-[160px]">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  <p className="text-foreground-400 text-xs leading-relaxed">{selectedLoc.aiExplanation}</p>
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {complianceIssues.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-foreground-600">
              <span className="w-12 h-12 flex items-center justify-center text-success text-3xl mb-2"><i className="ri-check-double-line"></i></span>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-foreground-100 text-xs font-medium">{issue.title}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                          issue.severity === 'critical' ? 'bg-error/20 text-error' :
                      ...
                      <p className="text-foreground-400 text-xs leading-relaxed mb-1">{issue.description}</p>
                      <div className="flex items-start gap-1.5 mt-2 p-2 rounded bg-background-50/30 border border-primary-500/5">
                        <span className="w-4 h-4 flex items-center justify-center text-success shrink-0 mt-px"><i className="ri-lightbulb-line text-xs"></i></span>
                        <p className="text-foreground-300 text-xs leading-relaxed">{issue.suggestion}</p>
                      </div>
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-foreground-600 text-[10px]">关联字段: {issue.field}</span>
                        <span className="text-foreground-600 text-[10px]">检查于: {issue.checkedAt}</span>
                      </div>
                    </div>
                  </div>
                </div>
