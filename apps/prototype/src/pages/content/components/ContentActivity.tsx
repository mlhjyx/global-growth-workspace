import type { ContentActivity } from '@/mocks/contentData';
...
const typeColors: Record<string, string> = {
  ...
  alert: 'text-warning',
  compliance: 'text-error',
};

export default function ContentActivityStream({ activities }: Props) {
  return (
    <div className="flex flex-col h-full">
      <div className="px-5 py-2.5 border-b border-primary-500/10 shrink-0 flex items-center gap-2">
        ...
        <h3 className="text-white text-xs font-semibold">内容活动流</h3>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-2.5">
        <div className="flex gap-3 overflow-x-auto">
          {activities.map((activity) => (
            <div
              key={activity.id}
              className="flex items-start gap-2 shrink-0 px-3 py-2 rounded-lg bg-white/3 border border-primary-500/5 min-w-[240px]"
            >
              <div className="w-7 h-7 rounded-full bg-primary-500/10 flex items-center justify-center shrink-0">
                <span className="text-foreground-300 text-[10px] font-medium">
                  {activity.userAvatar}
                </span>
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  <span
                    className={`w-3 h-3 flex items-center justify-center ${typeColors[activity.type]}`}
                  >
