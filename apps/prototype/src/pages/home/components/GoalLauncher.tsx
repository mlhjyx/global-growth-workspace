...
const GOAL_TEMPLATES = [
  {
    ...
    label: '找客户',
    desc: '按 ICP 发现并确认合格线索',
    to: '/accounts',
  },
  { icon: 'ri-store-2-line', label: '找经销商', desc: 'J-B 旅程：渠道伙伴招募', to: '/campaigns' },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mb-3">
        {GOAL_TEMPLATES.map((g) => (
          <Link
            key={g.label}
            to={g.to}
            className="rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2.5 hover:border-primary-500/40 hover:bg-primary-500/5 transition-colors"
          >
