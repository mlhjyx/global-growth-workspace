...
import { connectedPlatforms, availableIntegrations, apiTokens } from '@/mocks/integrationData';
...
export default function IntegrationsPage() {
  ...
  const statusIcon = (status: string) => {
    ...
    if (status === 'syncing')
      return <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4">
          <div>
            ...
            <p className="text-sm text-foreground-500 mt-0.5">
              {connectedPlatforms.filter((p) => p.status === 'connected').length} 个已连接平台 ·{' '}
              {apiTokens.length} 个 API Token
            </p>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {i.isEnterprise && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-ai-accent/15 text-ai-accent">
                              企业版
                            </span>
                          )}
                        </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
              <p className="text-xs text-foreground-500">
                管理 Global Growth Workspace API 访问令牌。Token
                前缀为可识别标识，完整密钥仅在创建时显示一次。
              </p>
              <button
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[
                        ...
                        <label
                          key={scope}
                          className="flex items-center gap-1.5 text-xs text-foreground-400 cursor-pointer"
                        >
                          <input
                            type="checkbox"
                            className="rounded border-primary-500/20 bg-background-100/50"
                          />
                          {scope}
                        </label>
                      ))}
              ...
              <table className="w-full min-w-[680px]">
                ...
                <tbody>
                  {apiTokens.map((token) => (
                    <tr
                      key={token.id}
                      className="border-b border-primary-500/5 hover:bg-white/[0.02]"
                    >
                      <td className="px-4 py-3 text-xs text-white">{token.name}</td>
                      <td className="px-4 py-3 text-xs text-foreground-500 font-mono">
                        {token.prefix}********
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {token.scopes.map((s) => (
                            <span
                              key={s}
                              className="text-[10px] px-1.5 py-0.5 rounded bg-background-200/40 text-foreground-600"
                            >
                              {s}
                            </span>
                          ))}
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`text-[11px] px-2 py-0.5 rounded-full ${token.isActive ? 'bg-green-500/15 text-green-400' : 'bg-foreground-700/15 text-foreground-600'}`}
                        >
