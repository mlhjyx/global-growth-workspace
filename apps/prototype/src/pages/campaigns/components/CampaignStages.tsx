            ...
            <div key={stage.id}>
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`w-4 h-4 flex items-center justify-center ${statusColor[stage.status]}`}
                  >
                    <i className={`${statusIcon[stage.status]} text-sm`}></i>
                  </span>
                  <span
                    className={`text-sm font-medium flex-1 ${isSelected ? 'text-white' : 'text-foreground-300'}`}
                  >
                  ...
                  <span
                    className={`w-3 h-3 flex items-center justify-center text-foreground-600 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}
                  >
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <span
                            className={`w-3.5 h-3.5 flex items-center justify-center rounded-sm mt-0.5 shrink-0 ${
                              isTaskDone
                          ...
                          >
                            <i
                              className={`${isTaskDone ? 'ri-check-line' : 'ri-checkbox-blank-line'} text-[10px]`}
                            ></i>
                          </span>
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-xs ${isTaskDone ? 'text-foreground-600 line-through' : 'text-foreground-400'}`}
                            >
                              {task.title}
                            </p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              <span className="text-foreground-700 text-[10px]">
                                {task.assignee}
                              </span>
                              {task.priority === 'high' && (
