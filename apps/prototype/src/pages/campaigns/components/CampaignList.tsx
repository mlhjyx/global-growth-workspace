...
import { CAMPAIGN_STATUS_UI } from '@/mocks/campaignData';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {filtered.map((campaign) => {
          ...
          const status = CAMPAIGN_STATUS_UI[campaign.status];
          return (
            <button
              key={campaign.id}
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-full ${status.bg} ${status.color} ${status.border} border whitespace-nowrap`}
                >
                  {status.label}
                </span>
              ...
              <p
                className={`text-sm font-medium truncate ${isSelected ? 'text-white' : 'text-foreground-300'}`}
              >
                {campaign.name}
              </p>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-foreground-600 text-[11px]">
                  <i className="ri-user-line mr-0.5 text-[10px]"></i>
                  {campaign.leads}
                </span>
                <span className="text-foreground-700">·</span>
                <span className="text-foreground-600 text-[11px]">
                  <i className="ri-file-text-line mr-0.5 text-[10px]"></i>
                  {campaign.content}
                </span>
                <span className="text-foreground-700">·</span>
                <span className="text-foreground-600 text-[11px]">
