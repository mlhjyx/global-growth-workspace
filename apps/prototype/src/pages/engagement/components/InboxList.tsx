...
export default function InboxList({ messages, selectedId, onSelect }: Props) {
  ...
  const highIntentCount = messages.filter(m => m.intentScore >= 80 && m.status !== 'converted').length;

  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-4 pb-2 shrink-0">
        <h3 className="text-white text-sm font-semibold mb-1 flex items-center gap-2">
          <span className="w-4 h-4 flex items-center justify-center text-primary-400"><i className="ri-message-3-line"></i></span>
          统一 Inbox
        </h3>
        <div className="flex items-center gap-2 mb-3">
          {unreadCount > 0 && (
            <span className="text-[10px] bg-primary-500/20 text-primary-300 px-1.5 py-0.5 rounded-full">{unreadCount} 未读</span>
          )}
          {highIntentCount > 0 && (
            <span className="text-[10px] bg-error/15 text-error px-1.5 py-0.5 rounded-full">{highIntentCount} 高意向</span>
          )}
              ...
              >
                <div className="flex items-start gap-2">
                  <div className="w-8 h-8 rounded-full bg-primary-500/15 flex items-center justify-center shrink-0 relative">
                    <span className="text-foreground-300 text-[10px] font-semibold">{msg.sender.avatar}</span>
                    {!msg.isRead && (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <span className="text-white text-xs font-medium truncate">{msg.sender.name}</span>
                      {msg.intentScore >= 80 && (
                        <span className="text-[10px] bg-error/15 text-error px-1 py-0 rounded-full shrink-0">
                          {msg.intentScore}
                        </span>
                      )}
                    </div>
                    <p className="text-foreground-500 text-[10px] truncate">{msg.sender.company} · {msg.sender.title}</p>
                    <p className="text-foreground-400 text-[11px] truncate mt-0.5">{msg.preview}</p>
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      <span className={`text-[9px] px-1 py-0 rounded flex items-center gap-0.5 ${platformConfig[msg.platform]?.color} bg-white/5`}>
                        <i className={`${platformConfig[msg.platform]?.icon} text-[8px]`}></i>
                        {typeLabels[msg.type]}
                      </span>
                      <span className="text-foreground-600 text-[9px]">{msg.timestamp.split('T')[1]?.slice(0, 5) || ''}</span>
                      {msg.status === 'converted' && (
                        <span className="text-[9px] text-success bg-success/10 px-1 py-0 rounded">已转线索</span>
                      )}
                    </div>
                  </div>
                </div>
