...
| 编号 | 决策 | 批准 |
|---|---|---|
| GDR-001 | **正式文档 PR 审查机制**：专项 Agent 评审团（架构/治理/安全等按需组建）出具分级报告（APPROVE / APPROVE_WITH_CHANGES / REQUEST_CHANGES；BLOCKER/MAJOR/MINOR/NOTE）→ Claude Code 修订并把结论落盘 PR → CI 验证 → **业务负责人最终批准；Agent 通过不构成合并授权**。BLOCKER 与 MAJOR 必须修复或登记阻塞后才可提请批准。代码类 PR 另需 Codex 独立审查（内部子 Agent 不替代外部独立审查）。 | 业务负责人 2026-07-04 |
| GDR-002 | **Foundation 运行时启动门修订**：以「BE-01α（Foundation 包中不依赖 Spike 的段）经业务负责人 R2 批准」取代「过 Gate 3」作为 Foundation 地基运行时（BE-02..05）编码启动门；BE-01β（Spike 依赖段）最晚 BE-04 前终审；纵向切片仍需 BE-01β。 | 业务负责人 2026-07-04（双轨计划批示） |
