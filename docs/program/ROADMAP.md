...
架构去风险线（并行 Spike）:     SPK-TMP/OPA/MGW 依双轨计划即刻可启动（本地 compose 即可，无需 M1 环境）
...
Track B 后端去风险:            计划生效（PR #19）；SPK-TMP/OPA/MGW ✅ 首轮真实验证（PR #21，验收卡回写 docs/oss/ 待办，
                             OPA 发现 4 项契约缺口→BE-06 前置 contracts R2 PR）；BE-01α ✅（PR #22）；
                             **BE-02 ✅ apps/api 骨架（本 PR，冒烟 12 项 11 PASS + #9 Windows PARTIAL）**；
                             节奏：M0-06 ✅（本 PR）→ Gate 1 用户测试（业务负责人执行 UAT）∥ BE-03；
                             BE-01β 待三卡终审；WIP=1 个 BE 运行时 PR；合并授权 GDR-003
```
...
| EPIC-M0-05 Engage/Opportunity/Insight | Inbox Context 面板、三级结果链视图、成本/归因补强、Expert/Ops 低保真占位 | ✅ [PR #24](https://github.com/mlhjyx/global-growth-workspace/pull/24) MERGED |
| EPIC-M0-06 旅程与验证基础 | J-A/J-B 串线（旅程层+导航条）、Gate 1 埋点（六类事件+看板点亮）、GitHub Pages 部署、UAT 脚本/采集表/Report 模板、原型冒烟测试入 CI | ✅ 本 PR（Preview: <https://mlhjyx.github.io/global-growth-workspace/>） |
...
后端地基与切片按 **docs/program/M1_BACKEND_FOUNDATION_PLAN.md**（BE-01α/β、SPK-TMP/OPA/MGW、BE-02..06、BE-09A..E）双轨推进；**FE-M1-01 Production Web Shell、FE-M1-02 First Vertical Slice Integration**（真实 Next.js 接真实 API）为产品级切片完成标准——API E2E 只是后端完成标准。
...
| 私有化是否进入 M3 | M2 架构冻结前 |
| 生产 IdP（OD-14） | M1 外部 Alpha 前 |
| 共享 Dev/Staging 环境建立 | BE-09 开始前 |
| 生产云架构 + Backup/RPO/RTO + 生产 CI/CD | M2 前（随云化决策，显式登记不消失） |
