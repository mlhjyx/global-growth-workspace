...
16. 第 15 部分：项目治理、研发准备与 Codex 执行规范
17. 附录：决策、需求、数据对象、OSS、验证项、证据、术语与覆盖矩阵
...
| 项目和 Codex | 第 15 部分 | 只定义交付流程和开发约束 |

## 0.5 追踪规则

- 所有顶层决策使用 `D-xxx`；
- 产品原则使用 `P-xx`；
- 功能需求使用域前缀，如 `FR-DAT-xxx`、`FR-MKT-xxx`；
- 架构决策使用 `ADR-xxx`；
- 页面使用 `PG-xxx`；
- 事件使用 `EVT-xxx`；
- 质量 Gate 使用 `GATE-xxx`；
...
| D-020 | v2.0 是新的唯一母本 | APPROVED | 原始 v1.1 和全量详细开发基线版仅作历史输入 |
...
| 风险 | 影响 | 应对 |
|---|---|---|
| 产品范围过大 | 无法形成价值闭环 | B2B、有限行业/市场、严格 P0 |
| 数据源不合规或不稳定 | 封禁、法律和质量风险 | Provider Adapter、供应商评审、来源记录 |
| AI 幻觉企业事实 | 品牌和法律风险 | RAG、Claim Checker、证据、审批 |
| 自动发布/外联误操作 | 声誉和账号风险 | Tool Policy、审批、预览、限流 |
| 单模型依赖 | 成本、可用性和质量波动 | Model Gateway、多供应商、Eval |
| 前端仍然复杂 | 激活低 | 目标式首页、渐进披露、用户测试 |
| 两个仓库技术债扩散 | 维护困难 | 执行内核封装、新核心独立、ADR |
| 归因数据不完整 | 无法证明价值 | 从 MVP 设计事件、UTM 和 CRM 回写 |
| 多租户隔离缺陷 | 严重安全事故 | 强制 tenant scope、自动化隔离测试 |
| 媒体生成成本失控 | 毛利下降 | 预估成本、预算、配额、缓存 |
| 平台 API 变化 | 发布失败 | Connector Contract、监控、降级、状态页 |
| 用户过度相信推荐 | 决策风险 | 来源、置信度、人工确认和免责声明 |
...
这些问题不阻碍文档形成，但必须在 Gate 0/Gate 1 关闭，不能交给 Codex 自行猜测。
...
Codex 开始某个 Epic 前必须拥有：

- Epic PRD 与 Requirement ID；
- 用户流程和页面状态；
- 数据 Schema；
- OpenAPI/AsyncAPI；
- 权限、风险和审计要求；
...
- 一个 PR 只解决一个清晰问题；
- 数据库迁移必须可回滚；
- 外部动作必须经过 Policy/Approval，不得绕过；
- 不得把 API Key、Prompt 或供应商参数硬编码；
- 新增 Tool 必须注册权限、风险、幂等和审计；
- 新增 AI 功能必须同时提交 Eval；
- 新增页面必须覆盖 Loading、Empty、Error、Permission、Partial Success；
- 修改公共接口必须更新契约测试和 ADR。
...
|---|---|---|---|
| UX-GOAL-001 | 用户可从业务目标创建流程 | 目标被映射为结构化 GoalSession，用户可编辑 | Web/BFF |
| KB-CLAIM-003 | 对外内容仅使用 Approved Claim | 未批准事实触发阻断或人工确认 | Knowledge/Policy |
| LEAD-PROV-002 | 每条 Lead 保留来源 | 页面可查看来源、时间、用途和证据 | Lead/Evidence |
| CAM-AUTH-004 | Campaign 授权有边界和有效期 | 超出人数、渠道、模板或时间需重新审批 | Campaign/Approval |
| AI-TOOL-001 | 模型不能直接执行高风险工具 | 所有调用先生成 ActionProposal | AI/Policy |
| ENG-INTENT-005 | 高风险消息自动升级 | 投诉、退订、价格/法律内容进入人工队列 | Engage |
| ANA-OUTCOME-001 | Dashboard 默认展示 Qualified Lead、SAO 和 Verified Outcome | 可下钻到 Lead、Touchpoint、Campaign 和 Evidence | Analytics |
| OPS-KILL-001 | 管理员可停止外部执行 | 停止后不再创建新发送/发布任务 | Operations |

## 15.15 变更控制

任何影响产品定位、数据权利、核心对象、外部动作、Release Gate 或主架构的变更必须提交 Product Decision Record 或 ADR，说明问题、选项、证据、影响、迁移、回滚和 Owner。未经批准，Codex 和研发不得自行改变状态机、权限、数据保留、Provider 顺序或自动化边界。
...
| 研发、Codex 和文档治理 | 第 0、15、附录 L | APPROVED | 单一责任、稳定 ID、DoR/DoD、变更和回归检查。 |
...
本版完整替代此前的 v2.0 正式重构版、“完整蓝图版”和“完整校正重构版”。后续拆分模块 PRD、页面规格、架构 ADR、API、数据字典、测试和 Codex 任务时，必须引用本版需求 ID。
...
- v2.0 取代原始 v1.1 和全量详细开发基线版，成为唯一产品母本；
