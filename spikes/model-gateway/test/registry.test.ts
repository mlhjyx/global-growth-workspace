...
describe('PromptRegistry（版本化，v1 来自真实 prompt.v1.md）', () => {
  it('v1 系统指令从 prompt.v1.md 提取，含铁律关键句；draft 不服务缺省解析（Codex 3521756913）', () => {
    const reg = new PromptRegistry();
    const entry = registerCompanyUnderstandingPromptV1(reg);
    expect(entry.version).toBe('v1');
    expect(entry.releaseStatus).toBe('draft'); // prompt.v1.md：Gate 3 前为 draft
    expect(entry.system).toContain('企业理解提取器');
    expect(entry.system).toContain('只输出资料中有证据的内容');
    // 缺省解析只落 released：无已发布版本 → fail-closed 抛错；钉住 draft 版本仍可用
    expect(() => reg.resolve('company-understanding/prompt')).toThrow(UnknownPromptError);
    expect(reg.resolve('company-understanding/prompt', 'v1').system).toBe(entry.system);
    // 发布后缺省解析生效
    reg.release('company-understanding/prompt', 'v1');
    expect(reg.resolve('company-understanding/prompt').version).toBe('v1');
  });

  it('缺省解析 = 最新 released（后注册的 draft 不越位）；钉住 v1 仍可用；重复注册同版本被拒（版本不可变）', () => {
    const reg = new PromptRegistry();
    ...
    // draft v2 晚于 released v1 注册：缺省解析仍是 v1；v2 发布后才切换
    expect(reg.resolve('company-understanding/prompt').version).toBe('v1');
    reg.release('company-understanding/prompt', 'v2');
