...
已关闭的决策在 `docs/decisions/` 以 PDR 记录：

- ~~1. 首批聚焦的 2 个行业~~ → **已关闭（PDR-001）：光伏能源、建材**
- ~~2. 首批 2 个目标国家/地区~~ → **已关闭（PDR-001）：东南亚、非洲**（具体国家由 Global Market Scan 与试点客户收敛，每区域 2-3 个深度 Market Pack）
...
注意：行业/国家在**产品架构上永远由客户决定**（Core + Pack，D-004）；PDR-001 只定义 M2 试点验证范围与首批做深的 Pack。

命中以上任一项时：停止，说明缺哪项决策、它阻断了什么，请业务负责人拍板。

## 合并纪律（PDR-004 后：服务端保护为第一道防线，本纪律为第二道）

仓库 2026-07-04 已转公开（PDR-004），main 启用服务端 ruleset `main-protection`：仅 PR 可合入、Required Check=verify、评论 thread 必须解决、仅 squash、禁 force push/删除。直推 main 与红灯合并已被服务端阻止。以下纪律继续执行：

1. **不尝试绕过或修改 ruleset**；ruleset 变更属 R2（需业务负责人批准）。
2. **合并前**：`set -euo pipefail`；用 `gh pr view --json state,mergeStateStatus,statusCheckRollup` 完整读取检查结果，**禁止用 `| head` 截断后判断**；确认非 Draft、检查全绿、Codex 审查评论已核实处理、无未解决冲突。
